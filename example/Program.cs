...
namespace NIDE
{
    ...
    class Program
    {
        ...
        static void main_loop()
        {
            ...
            views.null_window(true, 50, 85, false, 1 * views.WIDTH / 100, 6, ConsoleColor.Red, visable2);
            views.null_window(true, 50, 85, false, 50 * views.WIDTH / 100, 6, ConsoleColor.DarkBlue, visable2);
            for (int i = 0; i <= 10; i++)
            {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (Console.KeyAvailable)
                {
                    ...
                    else if (Console.ReadKey().Key == ConsoleKey.LeftArrow && views.column_options > 0)
                    {
                        views.move_left();
                        main_loop();
                    }
                    else if (Console.ReadKey().Key == ConsoleKey.RightArrow && views.column_options < views.all_options-1)
                    {
                        views.move_right();
                        main_loop();
